...
function fixture(status = 'VERIFIED') {
  return {
    apiVersion: 'btc-shadow-dashboard-readonly/2.1',
    ...
    adaptiveGrid: {
      protocol: 'btc-adaptive-grid-dashboard-evidence/1.0',
      generatedAt: '2026-09-06T06:00:00.000Z',
      status: 'COLLECTING',
      targetDecisions: 100,
      totalDecisions: 1,
      remainingDecisions: 99,
      progressPct: 1,
      modes: { gridActive: 1, gridDefensive: 0, abstain: 0 },
      maturedOutcomes: 0,
      completedCycles: 0,
      netPnl: 0,
      firstReviewReady: false,
      evidenceBasis: 'VERIFIED_APPEND_ONLY_PROSPECTIVE_CHAIN',
      safety: { shadowMode: true, spotOnly: true, readOnly: true, sendToExchange: false, realOrderCreated: false, formalTrialMutation: false, countsTowardFormal90D: false, productionEligible: false },
    },
  };
}
...
test('cache persists bounded projection and prospective Adaptive Grid evidence', () => {
  const s = storage();
  ...
  assert.equal(cached.data.adaptiveGrid.totalDecisions, 1);
  assert.equal(cached.data.adaptiveGrid.targetDecisions, 100);
  assert.equal(cached.data.adaptiveGrid.evidenceBasis, 'VERIFIED_APPEND_ONLY_PROSPECTIVE_CHAIN');
  assert.equal(cached.data.adaptiveGrid.safety.countsTowardFormal90D, false);
});
...
test('fresh VERIFIED cache remains readable', () => {
  ...
  const cached = readVerifiedSnapshot(s, 'k', TRIAL_ID, 90, now);
  assert.equal(cached?.trial.completedDays, 16);
  assert.equal(cached?.adaptiveGrid.totalDecisions, 1);
});
