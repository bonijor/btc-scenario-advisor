...
assert.match(bootstrap, /const Q=window\.__BTC_PERFORMANCE_QA__===true/);
...
assert.match(bootstrap, /else\{addEventListener\('load',z\);addEventListener\('pageshow',z\);document\.fonts/);
...
assert.match(accessibility, /window\.__BTC_PERFORMANCE_QA__ = performanceQa;/);
