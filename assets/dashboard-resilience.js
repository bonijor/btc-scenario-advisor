...
export function snapshotIsSafe(data,trialId){
  if(!data||typeof data!=='object'){renderGrid(null,null);return false}
  ...
  renderGrid(valid?data.adaptiveGrid:null,valid?data.adaptiveGridV2:null);
  return valid;
}
...
export function readVerifiedSnapshot(storage,key,trialId,requiredDays=90,now=Date.now()){
  try{
    ...
    renderGrid(null,null);storage.removeItem(key);
  }catch{renderGrid(null,null);try{storage.removeItem(key)}catch{}}
  return null;
}
