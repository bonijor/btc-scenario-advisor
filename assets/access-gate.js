(() => {
        ...
        <div class="authGateShell">
          ...
          <div class="authGateCard">
            ...
            <section class="authGateVerify" data-gate-panel="verify" hidden>
              <div class="authGateVerifyIcon" aria-hidden="true">✉</div><h2>Verificá tu email</h2><p id="gateVerifyText">Te enviamos un enlace de verificación. El dashboard se abrirá cuando tu correo esté confirmado.</p>
              <div class="authGateForm"><button class="authGateButton" id="gateCheckVerification" type="button">YA VERIFIQUÉ</button><button class="authGateButton secondary" id="gateResendVerification" type="button">Reenviar verificación</button></div>
  ...
  function setMessage(text = '', type = '') { const el = $('gateMessage'); if (el) { el.textContent = text; el.className = `authGateMessage${type ? ` ${type}` : ''}`; } }
  ...
  function setBusy(busy) {
    ...
    if (primary && busy) primary.setAttribute('aria-busy', 'true'); else primary?.removeAttribute('aria-busy');
  }
  function authErrorMessage(error) {
    const code = String(error?.code || '');
    const map = {
      'auth/invalid-credential': 'Email o contraseña incorrectos.', 'auth/invalid-email': 'Revisá el formato del email.', 'auth/user-disabled': 'Esta cuenta está deshabilitada.',
      'auth/email-already-in-use': 'Ese email ya tiene una cuenta.', 'auth/weak-password': 'La contraseña no cumple la seguridad mínima.', 'auth/popup-closed-by-user': 'La ventana de Google se cerró antes de completar el acceso.',
      'auth/popup-blocked': 'El navegador bloqueó la ventana de Google.', 'auth/too-many-requests': 'Demasiados intentos. Esperá unos minutos y probá nuevamente.', 'auth/network-request-failed': 'No pudimos conectar con el servicio de identidad.',
    };
  ...
  function beginDashboardLoading() {
    ...
    const api = $('apiStatus'); const market = $('marketStatus'); let observer = null;
    const ready = () => {
      const apiText = String(api?.textContent || '').trim(); const marketText = String(market?.textContent || '').trim();
      if (apiText && apiText !== '--' && marketText && marketText !== '--') { document.body.classList.remove('dashboard-loading'); observer?.disconnect(); }
    };
    ...
    ready(); setTimeout(() => { document.body.classList.remove('dashboard-loading'); observer?.disconnect(); }, 10000);
  }
  function grant(user) {
    state.user = user;
    document.body.classList.add('auth-granted', 'auth-gate-ready'); document.body.classList.remove('auth-denied'); document.body.style.overflow = '';
    setProtectedLocked(false); const gate = $('authGate'); if (gate) gate.hidden = true; beginDashboardLoading();
    window.dispatchEvent(new CustomEvent('btc:auth-granted', { detail: { uid: user.uid, emailVerified: true } }));
  }
  function lock(user = null) {
    state.user = user; document.body.classList.remove('auth-granted'); document.body.classList.add('auth-gate-ready', 'auth-denied'); document.body.style.overflow = 'hidden'; setProtectedLocked(true);
    const gate = $('authGate'); if (gate) gate.hidden = false;
    if (user && user.emailVerified !== true) { if ($('gateVerifyText')) $('gateVerifyText').textContent = `Enviamos la verificación a ${user.email || 'tu email'}. Confirmala para abrir el Shadow Lab.`; setMode('verify'); }
    else setMode('login');
  }
  function handleAuthState(user, error) {
    if (error) { lock(null); setMessage('La sesión no pudo validarse. El dashboard permanece bloqueado.', 'error'); return; }
    if (user?.emailVerified === true) grant(user); else lock(user || null);
  }
  async function run(action) { if (state.busy || !state.adapter) return; setBusy(true); setMessage(''); try { await action(); } catch (error) { setMessage(authErrorMessage(error), 'error'); } finally { setBusy(false); } }
  function bindUi() {
    document.querySelectorAll('[data-gate-mode]').forEach((button) => button.addEventListener('click', () => setMode(button.dataset.gateMode)));
    document.querySelectorAll('[data-toggle-password]').forEach((button) => button.addEventListener('click', () => { const input = $(button.dataset.togglePassword); if (!input) return; const reveal = input.type === 'password'; input.type = reveal ? 'text' : 'password'; button.textContent = reveal ? '◌' : '◉'; button.setAttribute('aria-label', reveal ? 'Ocultar contraseña' : 'Mostrar contraseña'); }));
    $('gateLoginForm')?.addEventListener('submit', (event) => { event.preventDefault(); const email = $('gateLoginEmail').value.trim(); const password = $('gateLoginPassword').value; if (!email || !password) return setMessage('Completá email y contraseña.', 'error'); void run(() => state.adapter.signInEmail({ email, password })); });
    $('gateRegisterForm')?.addEventListener('submit', (event) => { event.preventDefault(); const name = $('gateRegisterName').value.trim(); const email = $('gateRegisterEmail').value.trim(); const password = $('gateRegisterPassword').value; if (!name || !email || !password) return setMessage('Completá nombre, email y contraseña.', 'error'); void run(async () => { await state.adapter.registerEmail({ name, email, password }); setMessage('Cuenta creada. Revisá tu email para verificarla.', 'success'); }); });
    $('gateResetForm')?.addEventListener('submit', (event) => { event.preventDefault(); const email = $('gateResetEmail').value.trim(); if (!email) return setMessage('Ingresá el email de tu cuenta.', 'error'); void run(async () => { await state.adapter.resetPassword(email); setMode('login'); if ($('gateLoginEmail')) $('gateLoginEmail').value = email; setMessage('Te enviamos un enlace para recuperar la contraseña.', 'success'); }); });
    $('gateGoogleButton')?.addEventListener('click', () => { void run(() => state.adapter.signInGoogle()); });
    $('gateCheckVerification')?.addEventListener('click', () => { void run(async () => { const user = await state.adapter.refreshCurrentUser(); if (user?.emailVerified === true) grant(user); else setMessage('Todavía no aparece verificado. Abrí el enlace del email y volvé a comprobar.', 'error'); }); });
    $('gateResendVerification')?.addEventListener('click', () => { void run(async () => { await state.adapter.sendVerification(); setMessage('Enviamos un nuevo email de verificación.', 'success'); }); });
  ...
  async function start() {
    ...
    try {
      ...
      else { const module = await import('./firebase-auth.js'); state.adapter = await module.createFirebaseAuthAdapter({ onState: handleAuthState }); }
      const current = state.adapter.getCurrentUser?.(); if (current) handleAuthState(current, null); else if (state.mode === 'pending') lock(null);
    } catch (error) { lock(null); setMessage('Firebase Authentication no está disponible. El dashboard permanece bloqueado.', 'error'); }
  }
